...
public class OrderService(AppDbContext context, IConfiguration configuration, StockMovementService stockMovements)
{
    ...
    public async Task<SaleDto> DeleteItemAsync(int company, int branch, int id, int itemId, long? version)
    {
        ...
        if (sale.Items.Count == 1 && sale.Payments.Count > 0)
            throw new SaleException("No se puede eliminar el último producto de una venta con pagos registrados.", 409);
    ...
    public async Task<SaleDto> ApplyDiscountAsync(int company, int branch, int id, ApplyDiscountRequest request)
    {
        ...
        ValidateDiscount(type, value); sale.DescuentoId = request.DiscountId; sale.DescuentoNombre = name; sale.DescuentoTipo = type; sale.DescuentoValor = value; Changed(sale); Recalculate(sale);
    ...
    { var sale = await Editable(company, branch, id, version); sale.DescuentoId = null; sale.DescuentoNombre = sale.DescuentoTipo = null; sale.DescuentoValor = null; Changed(sale); Recalculate(sale); await context.SaveChangesAsync(); return (await GetAsync(company, branch, id))!; }

    public async Task<SaleDto> AddPaymentsAsync(int company, int branch, int user, int id, AddPaymentsRequest request)
    {
        var sale = await Editable(company, branch, id, request.Version); var paid = sale.Payments.Sum(p => p.Amount); var additions = request.Payments.Sum(p => p.Amount);
        if (request.Payments.Any(p => p.Amount <= 0)) throw new SaleException("Los importes de pago deben ser mayores que cero.");
        if (Money(paid + additions) > sale.Total) throw new SaleException("Los pagos superan el total de la venta.");
        foreach (var payment in request.Payments)
        {
            var type = await context.PaymentTypes.AsNoTracking().FirstOrDefaultAsync(p => p.Id == payment.PaymentTypeId && p.Activo && (p.IdEmpresa == null || p.IdEmpresa == company)) ?? throw new SaleException("Tipo de cobro no válido.", 404);
            sale.Payments.Add(new SalePayment { PaymentTypeId = type.Id, PaymentTypeName = type.Nombre, Amount = Money(payment.Amount), Fecha = DateTime.UtcNow, UsuarioId = user, Reference = payment.Reference?.Trim() });
        }
        sale.Version++; await context.SaveChangesAsync(); return (await GetAsync(company, branch, id))!;
    }

    public async Task<SaleDto> CheckoutAsync(int company, int branch, int user, int id, long? version)
    {
        await using var tx = await context.Database.BeginTransactionAsync(IsolationLevel.Serializable); var sale = await Editable(company, branch, id, version);
        if (sale.Commands.Count == 0 || sale.Estado == "ConCambios") throw new SaleException("Debe enviar la comanda y sus cambios antes de cobrar.", 409);
    ...
    public async Task<SaleDto> CancelAsync(int company, int branch, int id, long? version)
    {
        var sale = await Editable(company, branch, id, version); if (sale.Payments.Count > 0) throw new SaleException("No se puede cancelar una venta con pagos registrados.", 409);
        sale.Estado = "Cancelada"; sale.FechaCierre = DateTime.UtcNow; sale.Version++; sale.Table.Status = "Libre";
        foreach (var joined in sale.JoinedTables) (await context.Tables.FirstAsync(t => t.Id == joined.TableId)).Status = "Libre";
        await context.SaveChangesAsync(); return (await GetAsync(company, branch, id))!;
    }

    public async Task<SaleDto> MoveAsync(int company, int branch, int id, MoveTableRequest request)
    {
        await using var tx = await context.Database.BeginTransactionAsync(IsolationLevel.Serializable); var sale = await Editable(company, branch, id, request.Version); await ValidateFreeTable(company, branch, request.TableId, id);
    ...
    public async Task<SaleDto> JoinAsync(int company, int branch, int id, JoinTableRequest request)
    {
        ...
        var other = await BaseQuery().FirstOrDefaultAsync(v => v.IdEmpresa == company && v.IdSucursal == branch && ActiveStates.Contains(v.Estado) && v.TableId == request.TableId);
    ...
    private async Task<Order> Editable(int company, int branch, int id, long? version = null)
    {
    ...
    private static void Recalculate(Order sale) { sale.Subtotal = Money(sale.Items.Sum(i => i.Subtotal)); sale.ImporteDescuento = sale.DescuentoValor.HasValue ? DiscountAmount(sale.Subtotal, sale.DescuentoTipo!, sale.DescuentoValor.Value) : 0; sale.Total = Math.Max(0, Money(sale.Subtotal - sale.ImporteDescuento)); }
    ...
    private static SaleDto Map(Order v) => new() { Id = v.Id, Number = v.Numero, TableId = v.TableId, WaiterId = v.MozoId, Status = v.Estado, OpenedAt = v.FechaApertura, ClosedAt = v.FechaCierre, Subtotal = v.Subtotal, DiscountAmount = v.ImporteDescuento, Total = v.Total, Version = v.Version, Items = v.Items.Select(i => new SaleItemDto(i.Id, i.ProductId, i.ProductName, i.UnitPrice, i.Quantity, i.Subtotal, i.Comment, i.Estado)).ToList(), Discount = v.DescuentoValor.HasValue ? new { id = v.DescuentoId, name = v.DescuentoNombre, type = v.DescuentoTipo, value = v.DescuentoValor, amount = v.ImporteDescuento } : null, Payments = v.Payments.Select(p => (object)new { id = p.Id, paymentTypeId = p.PaymentTypeId, name = p.PaymentTypeName, amount = p.Amount, date = p.Fecha, reference = p.Reference }).ToList(), JoinedTableIds = v.JoinedTables.Select(j => j.TableId).ToList() };
    private static object Document(Order v, bool receipt) => new { type = receipt ? "Comprobante" : "TicketCuenta", companyId = v.IdEmpresa, saleNumber = v.Numero, date = v.FechaCierre ?? v.FechaApertura, table = v.Table.Nombre, items = v.Items.Select(i => new { quantity = i.Quantity, description = i.ProductName, unitPrice = i.UnitPrice, subtotal = i.Subtotal }), itemCount = v.Items.Sum(i => i.Quantity), v.Subtotal, discount = v.ImporteDescuento, v.Total, payments = v.Payments.Select(p => new { name = p.PaymentTypeName, amount = p.Amount }), disclaimer = receipt ? "Documento no válido como factura" : null };
