...
public sealed class PrintJobWorker : BackgroundService
{
    ...
    private static byte[] BuildCommandTicket(Order sale, string waiterName)
    {
        ...
        text.AppendLine(Center($"Pedido Nro: {command?.Numero ?? sale.Numero}", width));
        if (command is not null)
            text.AppendLine(Center(command.Tipo, width));
        ...
        text.AppendLine(string.IsNullOrWhiteSpace(sectorName)
            ? $"Mesa: {tableName}"
            : $"Mesa: {tableName} ({sectorName})");
        text.AppendLine($"Venta: {sale.Numero}");
        text.AppendLine($"Fecha: {(command?.Fecha ?? DateTime.UtcNow).ToLocalTime():dd/MM/yyyy HH:mm}");
        text.AppendLine($"Mozo: {waiterName}");
        text.AppendLine(new string('-', width));
        text.AppendLine("Cant. Descripcion");
        text.AppendLine(new string('-', width));
        ...
        if (command is not null)
        {
            foreach (var line in command.Lines.OrderBy(x => x.Id))
            {
                ...
                var prefix = quantity > 0 && command.Tipo != "Inicial" ? "+" : string.Empty;
                text.AppendLine($"{prefix + FormatQuantity(quantity),-5} {line.ProductName}");
                if (!string.IsNullOrWhiteSpace(line.Comment))
                    text.AppendLine($"  OBS: {line.Comment}");
            }
        }
        else
        {
            ...
            {
                text.AppendLine($"{FormatQuantity(item.Quantity),-5} {item.ProductName}");
                if (!string.IsNullOrWhiteSpace(item.Comment))
                    text.AppendLine($"  OBS: {item.Comment}");
            }
        }
        ...
        string waiterName)
    {
        ...
        text.AppendLine();
        text.AppendLine($"Pedido Nro: {sale.Numero}");
        ...
        foreach (var item in activeItems)
        {
            ...
            if (!string.IsNullOrWhiteSpace(item.Comment))
                foreach (var observation in Wrap($"({item.Comment})", 36))
                    text.AppendLine($"{"",-6}{observation}");
        }
        ...
        text.AppendLine($"FORMA DE PAGO: {(string.IsNullOrWhiteSpace(paymentNames) ? "Sin informar" : paymentNames)}");
        text.AppendLine();
