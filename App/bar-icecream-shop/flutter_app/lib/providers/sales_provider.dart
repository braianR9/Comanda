...
class SalesProvider extends ChangeNotifier {
  ...
  Future<SaleSnapshot> synchronizeItems({
    ...
    for (final entry in existingItemIds.entries) {
      if (currentProducts.contains(entry.key)) continue;
  ...
  Future<SaleSnapshot> addPayments(int saleId, int version,
          List<({int paymentTypeId, double amount})> payments) async =>
      _sale(await http
  ...
  Future<List<SaleCatalogItem>> paymentTypes() =>
      _catalog('/api/sales-catalogs/payment-types');
        ...
        if (value is Map<String, dynamic>)
          SaleCatalogItem(
            ...
            value['name']?.toString() ??
                ...
                'Sin nombre',
            type: value['type']?.toString() ?? value['tipo']?.toString() ?? '',
