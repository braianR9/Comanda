...
namespace BarIceCreamShop.Api.Data
{
    public class AppDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Usuario>().HasOne(u => u.SectorAsignado).WithMany().HasForeignKey(u => u.IdSectorAsignado).OnDelete(DeleteBehavior.SetNull);
            modelBuilder.Entity<Usuario>().HasMany(u => u.MesasExcluidas).WithOne().HasForeignKey(m => m.IdUsuario).OnDelete(DeleteBehavior.Cascade);
            modelBuilder.Entity<UsuarioMesaExcluida>().HasKey(m => new { m.IdUsuario, m.IdMesa });
            modelBuilder.Entity<UsuarioMesaExcluida>().HasOne<Table>().WithMany().HasForeignKey(m => m.IdMesa).OnDelete(DeleteBehavior.Cascade);
            modelBuilder.Entity<GoogleSheetJob>().HasKey(j => j.Id);
