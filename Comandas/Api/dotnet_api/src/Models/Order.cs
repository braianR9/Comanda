...
public class Order
{
    ...
    /// "Pedido Nro" que se imprime en comandas y tickets: el de la primera comanda
    /// enviada a cocina (no el número de venta, que es un correlativo aparte).
    public int PrintedNumber => Commands
        .OrderBy(c => c.Fecha).ThenBy(c => c.Id)
        .Select(c => (int?)c.Numero).FirstOrDefault() ?? Numero;
}
...
public class SalePayment
{
    ...
    [Column("id_tarjeta")] public int? CardId { get; set; }
    [Column("importe_base", TypeName = "decimal(18,2)")] public decimal? BaseAmount { get; set; }
    [Column("tipo_ajuste")] public string AdjustmentType { get; set; } = "SinAjuste";
    [Column("porcentaje", TypeName = "decimal(5,2)")] public decimal Percentage { get; set; }
    [Column("importe_ajuste", TypeName = "decimal(18,2)")] public decimal AdjustmentAmount { get; set; }
